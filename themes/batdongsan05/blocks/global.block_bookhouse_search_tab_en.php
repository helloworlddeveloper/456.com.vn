...
if (! nv_function_exists('nv_block_search_en')) {
    ...
    function nv_block_config_search_en($module, $data_block, $lang_block)
    {	
        ...
        if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/blocks/block_block_bookhouse_search_tab_en.tpl')) {
            $block_theme = $global_config['module_theme'];
        } else {
            $block_theme = 'default';
        }
		
        $xtpl = new XTemplate('block_block_bookhouse_search_tab_en.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/blocks');
    ...
    function nv_block_config_search_submit_en($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    function nv_block_search_en($block_config, $module)
    {
        ...
        $themetype = 'block_block_bookhouse_search_tab_en.tpl';
		...
		// HƯỚNG BẤT ĐỘNG SẢN
		$list_huong = $db->query('SELECT * FROM ' . NV_PREFIXLANG . '_' . $site_mods[$module_a]['module_data'] . '_way')->fetchAll();
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        ...
        $content = nv_block_search_en($block_config, $module);
    }
}
