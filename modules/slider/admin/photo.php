...
if( ACTION_METHOD == 'delete' )
{
	...
	if( ! empty( $del_array ) )
	{
		...
		foreach( $del_array as $photo_id )
		{
			...
			$db->query( 'DELETE FROM ' . TABLE_SLIDER_NAME . '_photo WHERE photo_id = ' . ( int )$photo_id );
...
elseif( ACTION_METHOD == 'add' || ACTION_METHOD == 'edit' )
{
	...
	$result = $db_slave->query( 'SELECT group_id, title FROM ' . TABLE_SLIDER_NAME . '_group ORDER BY weight ASC' );
