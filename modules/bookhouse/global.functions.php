...
$array_type = $nv_Cache->db($sql, 'id', $module_name);
...
function nv_item_group($bid, $itemid, $is_del = 0, $exptime = 0)
{
    ...
    if ($result) {
        ...
        $db->query('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . ' SET group_config=' . $db->quote($group_config) . ' WHERE id=' . $itemid);
...
function nv_check_similar_content($content)
{
    ...
    if (!empty($array_config['similar_content'])) {
        ...
        $result = $db_slave->query('SELECT id, bodytext FROM ' . NV_PREFIXLANG . '_' . $module_data . ' WHERE addtime >= ' . $similar_time . ' ORDER BY addtime DESC');
...
function nv_get_projetcs($provinceid, $districtid, $wardid)
{
    ...
    } elseif ($array_config['project_type'] == 1 and isset($site_mods[$array_config['project_module_name']]) and !empty($array_config['project_table']) and !empty($array_config['project_id_field']) and !empty($array_config['project_title_field']) and !empty($array_config['project_provinceid_field']) and !empty($array_config['project_districtid_field']) and !empty($array_config['project_wardid_field'])) {
...
{
    global $db, $array_config, $module_data, $site_mods, $module_name, $module_info;

    if ($array_config['project_type'] == 0) {
        $rows = $db->query('SELECT id, title, alias FROM ' . NV_PREFIXLANG . '_' . $module_data . '_projects WHERE id=' . $projectid)->fetch();
    } elseif ($array_config['project_type'] == 1 and isset($site_mods[$array_config['project_module_name']]) and !empty($array_config['project_table']) and !empty($array_config['project_id_field']) and !empty($array_config['project_title_field']) and !empty($array_config['project_provinceid_field']) and !empty($array_config['project_districtid_field']) and !empty($array_config['project_wardid_field'])) {
        ...
        $rows['alias'] = change_alias($rows['title']);
    }
    $rows['link'] = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $module_info['alias']['project-detail'] . '/' . $rows['alias'] . '-' . $rows['id'];
    return $rows;
}
