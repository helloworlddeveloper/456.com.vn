...
list ($tid, $title, $alias, $description, $image, $keywords, $bodytext ) = array(
    0,
    '',
    '',
    '',
...
if (! empty($savecat)) {
    ...
    if (empty($alias)) {
        ...
        if ($tid == 0) {
            $sth = $db->prepare('INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . '_tags (numnews, alias, description, image, keywords, bodytext ) VALUES (0, :alias, :description, :image, :keywords, :bodytext )');
            $msg_lg = 'add_tags';
        } else {
            $sth = $db->prepare('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_tags SET alias = :alias, description = :description, image = :image, keywords = :keywords, bodytext = :bodytext  WHERE tid =' . $tid);
            $msg_lg = 'edit_tags';
        }
...
if ($tid > 0) {
    list ($tid, $alias, $description, $image, $keywords, $bodytext ) = $db->query('SELECT tid, alias, description, image, keywords, bodytext  FROM ' . NV_PREFIXLANG . '_' . $module_data . '_tags where tid=' . $tid)->fetch(3);
...
if (! empty($image) and file_exists(NV_UPLOADS_REAL_DIR . '/' . $module_upload . '/' . $image)) {
    $image = NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_upload;
}
