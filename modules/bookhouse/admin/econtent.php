...
if (! empty($row)) {
    foreach ($row as $value) {
        if (empty($array_config['payport']) and ($value['action'] == 'upgrade_group' or $value['action'] == 'group')) {
            continue;
        }
        $is_show = 1;
...
if (! $is_show) {
    $contents = nv_theme_alert($lang_module['econtent_empty'], $lang_module['econtent_empty']);
