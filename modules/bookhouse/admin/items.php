...
if ($nv_Request->isset_request('add', 'get') or $nv_Request->isset_request('edit', 'get')) {
    ...
    if ($nv_Request->isset_request('submit', 'post')) {
        ...
        $data['alias'] = ($alias == '') ? strtolower(change_alias($data['title'])) : strtolower(change_alias($alias));
        $data['code'] = $nv_Request->get_string('code', 'post', '');
        $data['catid'] = $nv_Request->get_int('catid', 'post', 0);
        $data['phong_khach'] = $nv_Request->get_int('phong_khach', 'post', 0);
        $data['phong_ngu'] = $nv_Request->get_int('phong_ngu', 'post', 0);
        $data['phong_tam'] = $nv_Request->get_int('phong_tam', 'post', 0);
	...
	$data['noi_that'] = implode( ',', $noi_that );
        ...
        } elseif (empty($data['hometext'])) {
            $error = $lang_module['items_error_hometext'];
        ...
        if (empty($error)) {
            ...
            $data['homeimgthumb'] = 0;
            if (! nv_is_url($data['homeimgfile']) === true) {
            ...
            if ($id > 0) {
                ...
                $stmt->bindParam(':phong_khach', $data['phong_khach'], PDO::PARAM_INT);
                $stmt->bindParam(':phong_ngu', $data['phong_ngu'], PDO::PARAM_INT);
                $stmt->bindParam(':phong_tam', $data['phong_tam'], PDO::PARAM_INT);
                ...
                $sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . ' (title, alias, catid, phong_khach, phong_ngu, phong_tam, hometext, bodytext, admin_id, addtime, edittime, exptime, code, area, size_v, size_h, price, price_time, money_unit, typeid, projectid, way_id, legal_id, homeimgfile, homeimgthumb, 
                homeimgalt, front, road, noi_that, tien_ich, structure, type, provinceid, districtid, wardid, address, maps, inhome, allowed_comm, hitstotal, showprice, contact_fullname, contact_email, contact_phone, contact_address, ordertime)
				VALUES (:title, :alias, :catid, :phong_khach, :phong_ngu, :phong_tam, :hometext, :bodytext, :admin_id, ' . NV_CURRENTTIME . ', ' . NV_CURRENTTIME . ', :exptime,
				:code, :area, :size_v, :size_h, :price, :price_time, :money_unit, :typeid, :projectid, :way_id, :legal_id, :homeimgfile, ' . $data['homeimgthumb'] . ', :homeimgalt, :front, :road, :noi_that, :tien_ich, :structure, :type, :provinceid, :districtid, :wardid, :address, :maps, :inhome,
                ...
                $data_insert['phong_khach'] = $data['phong_khach'];
                $data_insert['phong_ngu'] = $data['phong_ngu'];
                $data_insert['phong_tam'] = $data['phong_tam'];
...
if(!empty($ds_tienich))
{
  ...
  // print_r($data);die;
    ...
    $is_search = false;
    $where = $status = '';
    $status = $queue ? ' AND is_queue=1' : ' AND is_queue=0';
    $array_data = array();
    ...
    $db->select('id, title, alias, catid, price, provinceid, districtid, wardid, money_unit, status, is_queue, status_admin, addtime')
        ->order('addtime DESC')
        ->limit($per_page)
    ...
    if (! empty($array_data)) {
        ...
        foreach ($array_data as $row) {
            if (defined('NV_IS_SPADMIN') or in_array($row['provinceid'], $array_province_display_item)) {
                $row['price'] = ! empty($row['price']) ? nv_number_format($row['price']) : $lang_module['items_price_contact'];
