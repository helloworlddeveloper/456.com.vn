...
$table_name = NV_PREFIXLANG . '_' . $module_data . '_categories';
...
{
    global $db, $module_data, $table_name;
    
    $sql = 'SELECT id FROM ' . $table_name . ' WHERE parentid=' . $parentid . ' ORDER BY weight ASC';
    $result = $db->query($sql);
    $weight = 0;
    while ($row = $result->fetch()) {
        ++ $weight;
        $db->query('UPDATE ' . $table_name . ' SET weight=' . $weight . ' WHERE id=' . $row['id']);
    }
}
...
{
    global $db, $module_name, $module_data, $admin_info, $nv_Cache, $table_name;
    
    $sql = 'SELECT parentid, title FROM ' . $table_name . ' WHERE id=' . $catid;
    list ($p, $title) = $db->query($sql)->fetch(3);
    
    // $sql = 'DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . ' WHERE catid=' . $catid;
    // $db->query( $sql );
    
    $sql = 'SELECT id FROM ' . $table_name . ' WHERE parentid=' . $catid;
    $result = $db->query($sql);
    while (list ($id) = $result->fetch(3)) {
        nv_del_cat($id);
    }
    
    $sql = 'DELETE FROM ' . $table_name . ' WHERE id=' . $catid;
    $db->query($sql);
...
if ($nv_Request->isset_request('add', 'get')) {
    ...
    if ($nv_Request->isset_request('submit', 'post')) {
        ...
        $array['alias'] = ($array['alias'] == '') ? strtolower(change_alias($array['title'])) : strtolower(change_alias($array['alias']));
        ...
        if (empty($array['title'])) {
            ...
            if (! empty($array['parentid'])) {
                $sql = 'SELECT COUNT(*) AS count FROM ' . $table_name . ' WHERE id=' . $array['parentid'];
            ...
            if (! $is_error) {
                $stmt = $db->prepare('SELECT COUNT(*) FROM ' . $table_name . ' WHERE alias= :alias');
        ...
        if (! $is_error) {
            ...
            $sql = 'SELECT MAX(weight) AS new_weight FROM ' . $table_name . ' WHERE parentid=' . $array['parentid'];
            $new_weight = $db->query($sql)->fetchColumn();
            $new_weight = (int) $new_weight + 1;
            
            $sql = 'INSERT INTO ' . $table_name . ' (parentid, title, alias, description, groups_view, weight, status) VALUES (
				 ' . $array['parentid'] . ',
				 :title,
				 :alias,
				 :description,
...
if ($nv_Request->isset_request('edit', 'get')) {
    ...
    if ($nv_Request->isset_request('submit', 'post')) {
        ...
        if (empty($array['title'])) {
            ...
            if (! $is_error) {
                $stmt = $db->prepare('SELECT COUNT(*) FROM ' . $table_name . ' WHERE id!=' . $catid . ' AND alias= :alias');
                $stmt->bindParam(':alias', $array['alias'], PDO::PARAM_STR);
                $stmt->execute();
...
if ($nv_Request->isset_request('del', 'post')) {
    ...
    $sql = 'SELECT id, parentid FROM ' . $table_name . ' WHERE id=' . $catid;
    $result = $db->query($sql);
...
if ($nv_Request->isset_request('changeweight', 'post')) {
    ...
    $query = 'SELECT parentid FROM ' . $table_name . ' WHERE id=' . $catid;
    $row = $db->query($query)->fetch();
    if (empty($row))
        die('NO');
    
    $query = 'SELECT id FROM ' . $table_name . ' WHERE id!=' . $catid . ' AND parentid=' . $row['parentid'] . ' ORDER BY weight ASC';
    ...
    $db->query('UPDATE ' . $table_name . ' SET weight=' . $new . ' WHERE id=' . $catid);
...
if ($nv_Request->isset_request('changestatus', 'post')) {
    ...
    $query = 'SELECT status FROM ' . $table_name . ' WHERE id=' . $catid;
    $row = $db->query($query)->fetch();
    if (empty($row))
        die('NO');
    ...
    $sql = 'UPDATE ' . $table_name . ' SET status=' . $status . ' WHERE id=' . $catid;
    $db->query($sql);
...
$sql = 'SELECT * FROM ' . $table_name . ' WHERE parentid=' . $pid . ' ORDER BY weight ASC';
...
if ($pid) {
    $sql2 = 'SELECT title,parentid FROM ' . $table_name . ' WHERE id=' . $pid;
...
foreach ($_array_cat as $row) {
    $numsub = $db->query('SELECT COUNT(*) FROM ' . $table_name . ' WHERE parentid=' . $row['id'])->fetchColumn();
    if ($numsub) {
    ...
    $list[$row['id']] = array(
        'id' => (int) $row['id'],
        'title' => $row['title'],
        'titlelink' => NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=items' . '&amp;catid=' . $row['id'] . '&amp;search',
        'numsub' => $numsub,
        'parentid' => $parentid,
        'weight' => $weight,
