...
if (! $id) {
    ...
    if (defined('NV_IS_USER')) {
        $data['contact_fullname'] = nv_show_name_user($user_info['first_name'], $user_info['first_name']);
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    } elseif ((empty($data['price']) or ! is_numeric($data['price'])) and $data['showprice']) {
    ...
    if (empty($error)) {
        ...
        if (isset($_FILES['upload_fileupload']) and is_uploaded_file($_FILES['upload_fileupload']['tmp_name'])) {
            ...
            $upload = new NukeViet\Files\Upload($file_allowed_ext, $global_config['forbid_extensions'], $global_config['forbid_mimes'], $array_config['maxfilesize'], $width, $height);
    ...
    if (empty($error)) {
        ...
        if ($id > 0) {
            ...
            $sql = 'INSERT INTO ' . NV_PREFIXLANG . '_' . $module_data . ' (title, alias, catid, hometext, bodytext, admin_id, addtime, edittime, exptime, area, size_v, size_h, price, price_time, money_unit, typeid, projectid, way_id, legal_id, homeimgfile, homeimgthumb, homeimgalt, 
				...
				:area, :size_v, :size_h, :price, :price_time, :money_unit, :typeid, :projectid, :way_id, :legal_id, :homeimgfile, ' . $data['homeimgthumb'] . ', :homeimgalt, :front, :road, :structure, :type, :provinceid, :districtid, :wardid, :address, :maps, :inhome,
        ...
        $contents = nv_theme_alert($lang_module['insert_success_title'], $lang_module['insert_success_content'], 'info', $url_back, $lang_module['items']);
        include NV_ROOTDIR . '/includes/header.php';
        echo nv_site_theme($contents);
        include NV_ROOTDIR . '/includes/footer.php';
    }
...
if (defined('NV_EDITOR') and nv_function_exists('nv_aleditor')) {
    $data['bodytext'] = nv_aleditor('bodytext', '100%', '300px', $data['bodytext'], 'Basic');
} else {
