...
$db->select('id, title, alias, catid, code, price, money_unit, status, is_queue, status_admin, price_time, homeimgfile, homeimgthumb, hitstotal, ordertime, group_config')
...
if (! empty($array_data)) {
    foreach ($array_data as $row) {
        ...
        $row['cat'] = $array_cat[$row['catid']]['title'];
        ...
        $row['view_url'] = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $array_cat[$row['catid']]['alias'] . '/' . $row['alias'] . '-' . $row['id'] . $global_config['rewrite_exturl'];
        ...
        if (! empty($row['group_config'])) {
            ...
            foreach ($group_config as $groupid => $exptime) {
                if ($exptime == 0 or $exptime > NV_CURRENTTIME) {
                    $row['color'] = $array_groups[$groupid]['color'];
                    break;
                }
            }
        }
        ...
        if ($row['is_queue'] == 1) {
            $row['status_s'] = $lang_module['items_status_queue'];
