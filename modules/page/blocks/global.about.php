...
if (! nv_function_exists('nv_message_page')) {
    ...
    function nv_message_page($block_config)
    {
        ...
        if (! $is_show) {
            $sql = 'SELECT id,title,alias,bodytext,keywords,add_time,edit_time,image  FROM ' . NV_PREFIXLANG . '_' . $site_mods[$module]['module_data'] . ' WHERE status=1 ORDER BY rand() DESC';
